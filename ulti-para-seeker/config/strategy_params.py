...
_PARAMS_FILE_PATH = os.path.join(_CONFIG_DIR, 'current_backtest_config.json')
...
class StrategyParams:
    ...
    def __init__(self, **kwargs):
        ...
        """
        # 基础回测参数
        # 确保initial_capital参数被正确传递，不使用默认值100000
        self.initial_capital = kwargs.get('initial_capital')
        if self.initial_capital is None:
            self.initial_capital = 100000  # 只有在参数确实未传递时才使用默认值
